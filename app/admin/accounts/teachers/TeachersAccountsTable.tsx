...
const TeachersAccountsTableFC: React.FC<IStyledFC> = ({className}) => {
    ...
    const table = useMaterialReactTable({
        ...
        renderTopToolbarCustomActions: () => (
            <Button variant="contained" endIcon={<AddIcon />} onClick={(e) => router.push(`./add-account`)}>Add Record</Button>
        ),
    ...
    React.useEffect(() => {
        fetch("/api/get-teachers-account")
        .then(response => {
    ...
    React.useEffect(() => {
        console.log((88 * 0.2) + (40 * 0.4) + (85 * 0.4))
    }, [])
    return(
...
const TeachersAccountsTable = styled(TeachersAccountsTableFC)`
    display: flex;
    flex: 0 1 100%;
    overflow: hidden;
    padding: 5px;
