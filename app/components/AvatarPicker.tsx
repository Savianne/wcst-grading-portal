...
const uploadAvatar = (formData: FormData, onProgress: (progress: number) => void, abortController: AbortController) : Promise<TResponseFlag<{imageName: string}>> => {

    return new Promise<TResponseFlag<{imageName: string}>>(async (res, rej) => {
        try {
            const response: AxiosResponse = await axios.post(`http://localhost:3001/uploader/upload-avatar`, formData, {
            ...
            if(response.data.success) {
                res({success: true, data: response.data.data})
            } else {
                throw new Error();
            }
          } catch (error) {
                ...
                <DialogContent dividers>
                    {
                        ...
                        </DropZone> : <DropZone {...getRootProps()} style={{height: '300px'}}>
                            <input {...getInputProps()} />
                            <span className="icon">
